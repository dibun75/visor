        ...
        <div style={{ 
            ...
            <div style={{ display: 'flex', flexDirection: isSidebar ? 'column' : 'row', gap: '16px', width: '100%', maxWidth: isSidebar ? 'none' : '800px', margin: isSidebar ? '0' : '0 auto', pointerEvents: 'none' }}>
                <div className="glass-panel" style={{ padding: '16px', flex: 1, display: 'flex', alignItems: 'center', gap: '16px', pointerEvents: 'auto' }}>
                    ...
                    <div>
                        <div style={{ fontSize: '11px', color: 'var(--text-muted)', letterSpacing: '1px' }}>TOKENS PROCESSED</div>
                        <div style={{ fontSize: '20px', fontWeight: 600 }}>{Math.round(telemetry.burn / 4).toLocaleString()} <span style={{fontSize:'12px', color:'var(--text-muted)', display: isSidebar ? 'block' : 'inline'}}>{telemetry.workspaceName ? telemetry.workspaceName.toUpperCase() : 'THIS WORKSPACE'}</span></div>
                    </div>
                </div>

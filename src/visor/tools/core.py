...
def _build_nx_graph() -> nx.DiGraph:
    ...
    cursor.execute(
        "SELECT DISTINCT file_path FROM code_nodes WHERE file_path NOT LIKE '%node_modules%' AND file_path NOT LIKE '%.venv%'"
    )
...
def register_tools(mcp: FastMCP):
    ...
    def get_file_context(path: str) -> str:
        ...
        if not rows:
            return json.dumps(
                {
                    "error": f"No indexed nodes found for '{path}'. Has the file been saved?"
                }
            )
        symbols = [
            {
        ...
        return json.dumps(
            {"file_path": path, "symbol_count": len(symbols), "symbols": symbols}
        )
    ...
    def get_drift_report(
        ...
        if file_hashes:
            ...
            try:
                ...
                return DriftReport(
                    drift_detected=False, stale_files=[], severity="INFO"
                ).model_dump_json()
        ...
        report = DriftReport(
            drift_detected=bool(stale_files), stale_files=stale_files, severity=severity
        )
    ...
    def set_hud_focus(file_paths: List[str], intent: str) -> str:
        ...
        data = {"paths": file_paths, "intent": intent}
    ...
    def impact_analysis(file_path: str) -> str:
        ...
        if not G.has_node(file_path):
            return json.dumps(
                {"error": f"Node {file_path} not found in architecture graph."}
            )
        ...
        return json.dumps(
            {"target": file_path, "blast_radius": list(set(impacted_nodes))}
        )
    ...
    def trace_route(source: str, target: str) -> str:
        ...
        try:
            ...
            return json.dumps(
                {"error": "No architectural path found between these nodes."}
            )
    ...
    def get_dependency_chain(symbol: str) -> str:
        ...
        cursor = db_client.conn.cursor()
        cursor.execute(
            "SELECT file_path FROM code_nodes WHERE name LIKE ? LIMIT 1",
            (f"%{symbol}%",),
        )
        row = cursor.fetchone()
        if not row:
            return json.dumps(
                {
                    "error": f"Symbol '{symbol}' not found — cannot resolve dependency chain."
                }
            )
        source_file = row[0]
            ...
            return json.dumps(
                {
                    "source": source_file,
                    "chain": [],
                    "note": "No outgoing edges found. Run the file watcher to populate the edges table.",
                }
            )
        ...
        return json.dumps(
            {"symbol": symbol, "source_file": source_file, "dependency_chain": chain}
        )
    ...
    def add_custom_skill(
        name: str, description: str, content: str, strategy: Optional[str] = None
    ) -> str:
